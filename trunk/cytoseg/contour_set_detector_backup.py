...
#        self.contourTrainer.thresholdStep = 0.2
        ...
        # for mitochondria
        self.contourClassifier.numberOfThresholds = 7 #5 #2 #5 #1 #1 #4
        self.contourClassifier.firstThreshold = 30 #20 #160 #0.05 #0.4 #0.2
    
        # not for mitochondria
        #self.contourClassifier.numberOfThresholds = 1 #4
        #self.contourClassifier.firstThreshold = 0.4 #0.2
    
        self.contourClassifier.thresholdStep = 36 #40 #80 #40 #0.05
    ...
    def run(self, steps):
        ...
        if steps == 'classifyVoxels':
            ...
            #self.contourClassifier.runWriteContoursToImageStack()
            self.contourClassifier.runWriteInputContoursToBinaryImageStack()


        elif steps == 'inputContourListProbabilityFilter':

            self.contourClassifier.runLoadContourProcessingInputImage()
            self.contourClassifier.runContourListProbabilityFilter()
            ...
            self.contourClassifier.runLoadContourProcessingInputImage()
            self.contourClassifier.runInitialize()
            self.contourClassifier.runFill3DBlobsFromContourListsHighProbabilityOnly()
            self.contourClassifier.runWrite3DBlobsVolume()


        elif steps == 'accuracy':

            #self.contourClassifier.runLoadAccuracyCalcData()
            self.contourClassifier.runCalculateVoxelClassificationAccuracySingleComponent()
