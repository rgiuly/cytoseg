...
class BlobDetector:
    ...
    def __init__(self, parameterDict):
        ...
        # for mitochondria
        self.contourClassifier.numberOfThresholds = 4 #1 #4
        self.contourClassifier.firstThreshold = 0.2 #0.4 #0.2
    
        # not for mitochondria
        #self.contourClassifier.numberOfThresholds = 1 #4
        #self.contourClassifier.firstThreshold = 0.4 #0.2
    
        self.contourClassifier.thresholdStep = 0.2
    ...
    def run(self, runAllSteps=True):
        ...
        if runAllSteps:
            ...
            #self.contourClassifier.runStep(stepNumber)
            ...
            self.contourClassifier.runMakeContourLists()
            self.contourClassifier.loadItemsForViewing()
            self.contourClassifier.saveContourPathsToJinxFile()
            self.contourClassifier.run3DShellActiveContourToDetect3DBlobs()
            self.contourClassifier.runWrite3DBlobsVolume()
            #self.contourClassifier.runContourProbabilityFilter()
            #self.contourClassifier.run3DShellActiveContourToDetect3DBlobsHighProbabilityOnly()
            #self.contourClassifier.runVoxelTestSteps()
            #self.contourClassifier.runContourTestSteps()
            #self.contourClassifier.runMainLoop()
            self.app.MainLoop()
            ...
            #self.contourClassifier.loadItemsForViewing()
            #self.contourClassifier.loadOriginalVolume()
            #self.contourClassifier.loadVoxelClassificationResult()
            #self.contourClassifier.calculateVoxelClassificationAccuracy()
            #self.contourClassifier.calculateFinalAccuracy()
            #self.contourClassifier.calculateFinalAccuracyWithManualCorrection()
            #testContours()
            #self.contourClassifier.runFindContours()
            #self.contourClassifier.runMainLoop()
            self.app.MainLoop()
